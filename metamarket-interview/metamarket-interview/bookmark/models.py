...
class BookmarkItem(models.Model):
    ...
    def quicksort_by_site_name(cls, items):
        if len(items) <= 1:
            return items
        pivot = items[len(items) // 2]
        left = [item for item in items if item.site_name < pivot.site_name]
        middle = [item for item in items if item.site_name == pivot.site_name]
        right = [item for item in items if item.site_name > pivot.site_name]
        return cls.quicksort_by_site_name(left) + middle + cls.quicksort_by_site_name(right)